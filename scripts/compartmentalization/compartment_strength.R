...
c_score5K <- do.call(rbind, lapply(filepath, function(x) {
  ...
  df <- df[c(-1, -40), c(-1, -40)]
  BB <- sum(df[1:8, 1:8])     # ignore 2.5% of genomic bins with the lowest and highest E1 value.
  AA <- sum(df[31:38, 31:38]) # calculate the sum of observed-over-expected values in top 20% genomic bins with AA interactions.
  AB <- sum(df[1:8, 31:38])   # calculate the sum of observed-over-expected values in top 20% genomic bins with AB interactions etc.
  BA <- sum(df[31:38, 1:8])
  total <- (AA + BB) / (AB + BA)
  totalA <- (AA) / (AB + BA)
  totalB <- (BB) / (AB + BA)
...
ggsave("./compartment_strength_Fig2b.pdf", width = 6.5, height = 2.5)
...
ggsave("./Fig2/AA_BB_percentage_Fig2b.pdf", width = 6.5, height = 2)